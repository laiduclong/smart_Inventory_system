...
class ComputerIPC:
    ...
    def handle_client_message(self, conn, address):
        print("nhan duoc roi")
        tin_nhan = conn.recv(50).decode(FORMAT_STRING)
        print(tin_nhan)
    
    def server_listening(self):
        print(SERVER+"anh long")
    ...
    def read_serial(self):
        while self.is_run:
#không được chạy self.start() ở phía dưới nữa vì sever chỉ đc khởi tạo 1 lần
            # self.start()

            # print("đéo nhận được rồi")
            line = self.ser.readline().decode('utf-8').replace('\n','')
            print(line)
            # print("đéo nhận được rồi oki")
            if not self.is_run:
                break
        self.listen.stop()
    ...
    def send_operationMessage(self):
        ...
        if self.current_keys[0] == 79 and len(self.current_keys) > 1:
            if self.current_keys[1] < 51 and len(self.current_keys) == 3:
                ...
                print(message)
                self.ser.write((message + 'anh long').encode('utf-8'))
                self.current_keys = []

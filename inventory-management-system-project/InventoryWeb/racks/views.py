...
class UserViewSet(viewsets.ModelViewSet,viewsets.ViewSet, generics.ListAPIView, generics.CreateAPIView, generics.RetrieveAPIView, generics.UpdateAPIView):
    #viewsets.ModelViewSet cho phep xoa tren http://127.0.0.1:8000 neu khong cho viewsets.ModelViewSet len tren thi no se khong cho xoa
...
def search_users(request):
    query = request.query_params.get('query', None)
    if not query:  # Kiểm tra nếu query là None hoặc trống
...
def get_latest_operation_status(request, rack_id):
    try:
        if OperationStatus.objects.filter(rack_id=rack_id).order_by('-id')[:1]:
            latest_operation_status = OperationStatus.objects.filter(
                rack_id=rack_id).order_by('-id')[:1][0]
